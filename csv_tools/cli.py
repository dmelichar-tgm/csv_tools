...
"""
*******************************
Commandline Argument Controller
*******************************

This is a rather basic wrapper around Python's builtin
argparse module. It allows custom arguments to be set with
descriptions and designated executions.
"""
...
from csv_tools.exceptions import ColumnIdentifierError, RequiredHeaderError
...
class CSVToolsUtility(object):
    ...
    def __init__(self, args=None, output_file=None):
        """
        Perform argument processing and other setup for a CSVToolsUtility.
        """
    ...
    def add_arguments(self):
        ...
        """
        raise NotImplementedError('add_arguments must be provided by each subclass of CSVToolsUtility.')
    ...
    def main(self):
        ...
        """
        raise NotImplementedError(' must be provided by each subclass of CSVToolsUtility.')
    ...
    def _init_common_parser(self):
        ...
        if 'd' not in self.override_flags:
            self.argparser.add_argument('-d', '--delimiter', dest='delimiter', help='Delimiting character of the input CSV file.')
        if 't' not in self.override_flags:
            self.argparser.add_argument('-t', '--tabs', dest='tabs', action='store_true', help='Specifies that the input CSV file is delimited with tabs. Overrides "-d".')
        if 'q' not in self.override_flags:
            self.argparser.add_argument('-q', '--quotechar', dest='quotechar', help='Character used to quote strings in the input CSV file.')
        if 'u' not in self.override_flags:
            self.argparser.add_argument('-u', '--quoting', dest='quoting', type=int, choices=[0, 1, 2, 3], help='Quoting style used in the input CSV file. 0 = Quote Minimal, 1 = Quote All, 2 = Quote Non-numeric, 3 = Quote None.')
        if 'b' not in self.override_flags:
            self.argparser.add_argument('-b', '--doublequote', dest='doublequote', action='store_true', help='Whether or not double quotes are doubled in the input CSV file.')
        if 'p' not in self.override_flags:
            self.argparser.add_argument('-p', '--escapechar', dest='escapechar', help='Character used to escape the delimiter if --quoting 3 ("Quote None") is specified and to escape the QUOTECHAR if --doublequote is not specified.')
        if 'z' not in self.override_flags:
            self.argparser.add_argument('-z', '--maxfieldsize', dest='maxfieldsize', type=int, help='Maximum length of a single field in the input CSV file.')
        if 'e' not in self.override_flags:
            self.argparser.add_argument('-e', '--encoding', dest='encoding', default='utf-8', help='Specify the encoding the input CSV file.')
        if 'S' not in self.override_flags:
            self.argparser.add_argument('-S', '--skipinitialspace', dest='skipinitialspace', default=False, action='store_true', help='Ignore whitespace immediately following the delimiter.')
        ...
        if 'v' not in self.override_flags:
            self.argparser.add_argument('-v', '--verbose', dest='verbose', action='store_true', help='Print detailed tracebacks when errors occur.')

        # Output
        if 'l' not in self.override_flags:
            self.argparser.add_argument('-l', '--linenumbers', dest='line_numbers', action='store_true', help='Insert a column of line numbers at the front of the output. Useful when piping to grep or as a simple primary key.')

        # Input/Output
        if 'zero' not in self.override_flags:
            self.argparser.add_argument('--zero', dest='zero_based', action='store_true', help='When interpreting or displaying column numbers, use zero-based numbering instead of the default 1-based numbering.')
        ...
        def handler(t, value, traceback):
            if self.args.verbose:
                ...
                if t == UnicodeDecodeError:
                    sys.stderr.write(
                        'Your file is not "%s" encoded. Please specify the correct encoding with the -e flag. Use the -v flag to see the complete error.\n' % self.args.encoding)
                else:
                    sys.stderr.write('%s\n' % six.text_type(value))
    ...
    def print_column_names(self):
        ...
        rows = CSVToolsReader(f, **self.reader_kwargs)
    ...
    if isinstance(c, six.string_types) and not c.isdigit() and c in column_names:
        ...
        # Fail out if neither a column name nor an integer
            ...
            raise ColumnIdentifierError(
                'Column identifier "%s" is neither an integer, nor a existing column\'s name.' % c)
        ...
        if c >= len(column_names):
            raise ColumnIdentifierError('Index %i is beyond the last named column, "%s" at index %i.' % (
