...
"""
****************************
Insert a CSV into a Database
****************************

This script has three main features:
    -   generate SQL statements that insert a CSV into a DB with the given Database dialect
    -   send SQL statements to a DB and execute them
    -   create tables with specific names and insert data


The way this is achieved is by actually generating SQL-Statements
which are then sent to the DB and are executed. The currently
supported DBMS are PostgreSQL, MySQL, and SQLLite.

Things to know:
    -   You can insert multiple CSVs at the same time
    -   If you don't specify a DB, a SQLLite database will be created in your memory
    -   mFile names can be table names (be careful)
...
class InsertCSV(CSVToolsUtility):
    ...
    def add_arguments(self):
        self.argparser.add_argument(metavar="FILE", nargs='*', dest='input_paths', default=['-'], help='The CSV file(s) to operate on. If omitted, will accept input on STDIN.')
        ...
        self.argparser.add_argument('-i', '--dialect', dest='dialect', choices=sql.DIALECTS, help='Dialect of SQL to generate. Only valid when --db is not specified.')
        self.argparser.add_argument('--db', dest='connection_string', help='If present, a sqlalchemy connection string to use to directly execute generated SQL on a database.')
        self.argparser.add_argument('--query', default=None, help='Execute one or more SQL queries delimited by ";" and output the result of the last query as CSV.')
        self.argparser.add_argument('--insert', dest='insert', action='store_true', help='In addition to creating the table, also insert the data into the table. Only valid when --db is specified.')
        self.argparser.add_argument('--tables', dest='table_names', help='Specify one or more names for the tables to be created. If omitted, the filename (minus extension) or "stdin" will be used.')
        self.argparser.add_argument('--no-constraints', dest='no_constraints', action='store_true', help='Generate a schema without length limits or null checks. Useful when sampling big tables.')
        self.argparser.add_argument('--no-create', dest='no_create', action='store_true', help='Skip creating a table. Only valid when --insert is specified.')
        self.argparser.add_argument('--blanks', dest='blanks', action='store_true', help='Do not coerce empty strings to NULL values.')
